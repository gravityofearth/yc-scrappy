...
const Dashboard = () => {
  ...
  const handleOverview = async (profile: ProfileModel) => {
    setProfile(profile);
    setOverview(true);
    // Mark profile as read when opening
    try {
      const res = await fetch(`/api/profiles/${profile.userId}/read`, {
        method: "PATCH",
      });
      if (res.ok) {
    ...
    <>
      <div className="relative p-5 flex flex-1 flex-col gap-6 overflow-auto">
        ...
        <ProfileTable
          ...
          onContacted={(p) =>
            setProfiles((prev) =>
              prev.map((x) => (x.userId === p.userId ? { ...x, contact: true } : x))
            )
          }
        />
        <ProfileFooter
